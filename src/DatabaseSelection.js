...
class DatabaseSelection extends Component {
  ...
  formSubmit(event) {
    ...
    // .then(res =>{
     // this.deleteFromFirebase(this.state.urls) 
        axios.post("https://attribute-search-engine.herokuapp.com/faces/api/v1/upload", data, {
  ...
  handleChange = (e) => {
   ...
   handleUpload = () => {
    ...
    //setTimeout(() => {  console.log("sleep"); }, 8000);
      fetch("https://attribute-search-engine.herokuapp.com/faces/api/v1", options)
      .then(res => res.json())
      .then(data => {   
          this.setState({
          imgs_uploaded: false});  
      ...
      <Dropzone  class= "dropzone" accept='image/*' onDrop={acceptedFiles => { 
        ...
        {({getRootProps, getInputProps}) => (
          <section>
            ...
            <input {...getInputProps()} directory="" webkitdirectory="" type="file" />
              <p>Drag and drop image folder here, or click to select image files</p>
            </div>
            <aside>
              {/* <h4>Files</h4> */}
              {this.state.files.length!=0 ? <ul>{this.state.files}</ul> : <ul> <li><h4>Nothing uploaded</h4></li></ul>}
            </aside>
          </section>
        )}
      </Dropzone>
